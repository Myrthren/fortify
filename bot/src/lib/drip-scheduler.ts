...
import { sweep, weeklyReport } from "./drip";
...
function once(job: string, stamp: string): boolean {
  if (lastRun.get(job) === stamp) return false;
...
async function tick(client: Client) {
  ...
  try {
    if (minute === WELCOME_MINUTE && once("welcome", stamp)) {
      await sweep(client, "WELCOME");
    }
    if (minute === ACTIVATION_MINUTE && once("activation", stamp)) {
      await sweep(client, "ACTIVATION");
    }
    if (
      now.getUTCDay() === REPORT_DAY &&
      now.getUTCHours() === REPORT_HOUR &&
      minute === REPORT_MINUTE &&
      once("report", stamp)
    ) {
      await weeklyReport(client);
    }
  } catch (e) {
...
export function startDripScheduler(client: Client) {
  setInterval(() => void tick(client), 30 * 1000);
  console.log(
    `[drip] scheduler started — WELCOME :${WELCOME_MINUTE}, ACTIVATION :${ACTIVATION_MINUTE}, report Mon ${REPORT_HOUR}:${String(REPORT_MINUTE).padStart(2, "0")} UTC`
  );
}
