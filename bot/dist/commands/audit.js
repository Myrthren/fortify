...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .addStringOption((o) => o.setName("url").setDescription("Page URL to audit").setRequired(true));
async function execute(interaction) {
    const url = interaction.options.getString("url", true);
    await interaction.deferReply({ ephemeral: true });
    const user = await (0, usage_1.getOrCreateUser)(interaction.user.id, interaction.user.username);
    const limit = tiers_1.TIER_LIMITS[user.tier].monthlyAudits;
    const { ok, used } = await (0, usage_1.checkMonthly)(user.id, "audit", limit);
    if (!ok) {
    ...
    try {
        const result = await (0, audit_1.auditUrl)(url);
        await (0, usage_1.logGeneration)(user.id, "audit", url, JSON.stringify(result));
        const avgScore = (Object.values(result.scores).reduce((a, b) => a + b, 0) /
            Object.keys(result.scores).length).toFixed(1);
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: `Scores (avg ${avgScore}/10)`, value: scoresStr.slice(0, 1000) }, ...(topIssues.length
            ? [
                {
                    name: "Top issues",
                    value: topIssues
